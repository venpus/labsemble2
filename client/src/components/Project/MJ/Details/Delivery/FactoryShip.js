...
const FactoryShip = ({ project, onDateChange, handleMultipleUpdates, isAdmin, isAdminLoading }) => {
  ...
  useEffect(() => {
    if (project?.actual_order_date && project?.expected_factory_shipping_date) {
      ...
      // 출고상태가 변경되었는지 확인하고 DB에 저장
      if (project?.factory_shipping_status !== currentStatus) {
  ...
  }, [project?.actual_order_date, project?.expected_factory_shipping_date, project?.actual_factory_shipping_date, project?.factory_shipping_status]);
